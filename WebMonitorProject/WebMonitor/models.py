        ...
        if device.storage_osOID != None and device.storage_alloc_osOID != None and device.usedstorage_osOID != None:
            try:
                ...
                if device.used_storage != float(usedstorage_size*storage_alloc_size/GB):
                    mes = device.name + ' used storage percentage equal to ' + str.format(float(usedstorage_size*storage_alloc_size/GB)/float(storage_size*storage_alloc_size/GB),'.2f') + '% at ' +  pytz.utc.localize(datetime.datetime.utcnow()).strftime("%m/%d/%Y, %H:%M:%S")
